...
test_that("simple cases work correctly", {
  ...
  expect_equal(kNN_impute(x2, k=3, q=2, parallel= FALSE, verbose= FALSE)$x[5,],
               colMeans(x2, na.rm=T))
  expect_equal(kNN_impute(x2, k=3, q=2, parallel= TRUE, verbose= FALSE)$x[5,],
               colMeans(x2, na.rm=T))
  
  # parallel equivalence
  expect_equal(kNN_impute(x1, k=5, q=2, parallel= FALSE, verbose= FALSE),
               kNN_impute(x1, k=5, q=2, parallel= TRUE, verbose= FALSE))
  expect_equal(kNN_impute(x1, k=3, q=2, parallel= FALSE, verbose= FALSE),
               kNN_impute(x1, k=3, q=2, parallel= TRUE, verbose= FALSE))
  expect_equal(kNN_impute(x1, k=3, q=4, parallel= FALSE, verbose= FALSE),
               kNN_impute(x1, k=3, q=4, parallel= TRUE, verbose= FALSE))
  
  # returns matrix
  expect_true(is.matrix(kNN_impute(x1, k=3, q=2, parallel= FALSE, verbose= FALSE)$x))
  expect_true(is.matrix(kNN_impute(x1, k=3, q=2, parallel= TRUE, verbose= FALSE)$x))
